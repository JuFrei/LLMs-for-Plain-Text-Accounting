...
def check_transactions_compile(df, file_path, company):
    ...
    Returns:
    - pd.DataFrame: Updated DataFrame with a "Transaction_Compiles" column indicating compile status.
    """
    
    # Add "Transaction_Compiles" column if it does not exist
    if "Transaction_Compiles" not in df.columns:
        df["Transaction_Compiles"] = None
    ...
    for index, row in company_rows.iterrows():
        if pd.notna(row["Transaction"]):
            ...
            if errors:
                df.at[index, "Transaction_Compiles"] = 0

                error_messages = "\n".join([error.message for error in errors])
                df.at[index, "Transaction_Error"] = error_messages
            else:
                df.at[index, "Transaction_Compiles"] = 1
...
df = pd.read_csv("results/csv-files/scenarios_sampled.csv")
...
df.to_csv("results/csv-files/scenarios_compiled.csv", index=False)
