...
parser.add_argument("--max_tokens", type=int, default=8126,
                    help="Maximum number of tokens to generate, default: 8126")
...
def generate_chat(chat_history, llm, user_input, sampling_params):
    """
    ...
    Args:
        chat_history: List which serves as protocol for chats.
        llm: LLM which is used to generate text.
        user_input: User input that is appended to the chat history.
        sampling_params: Sampling parameters for LLM based on SamplingParams class.
    """
...
for ratio in ratios:
    for company in companies:
        ...
        for prompt_key, prompt_value in prompts["standardized_prompts"].items():
            generate_chat(chat_history, llm, prompt_value, sampling_params)
            print(f"Model generated response for: {prompt_key}")

        # Process example prompt specific to the ratio and generate response
        example_prompt_key = f"prompt6_{ratio}"
        example_prompt_content = prompts["example_prompts"][example_prompt_key]
        generate_chat(chat_history, llm, example_prompt_content, sampling_params)
        print(f"Model generated response for prompt6_{ratio}")

        # Process prompt7 for company balance sheet and generate response
        prompt7_key = f"prompt7_{company}"
        prompt7_content = prompts["balance_prompts"][prompt7_key]
        generate_chat(chat_history, llm, prompt7_content, sampling_params)
        print(f"Model generated response for {company} prompt7_{company}")

        # Process prompt8 for task 1 specific to the company and ratio and generate response
        prompt8_key = f"prompt8_{ratio}"
        prompt8_content = prompts["task1_prompts"][prompt8_key]
        generate_chat(chat_history, llm, prompt8_content, sampling_params)
        print(f"Model generated response for {company} prompt8_{ratio}")        

        # Generate model response for prompt 9 
        generate_chat(chat_history, llm, prompts["prompt9"], sampling_params)
        print(f"Model generated response for prompt9")

        # Create output directory
        output_dir = "/results/model_outputs"
        ...
        output_dir = os.path.join("/results/model_outputs", model_name)
        os.makedirs(output_dir, exist_ok=True)

        # Save output to a file with the naming scheme: MODEL-RATIO-COMPANY.json
