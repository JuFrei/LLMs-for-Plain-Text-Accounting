...
**Supervisors:** Prof. Dr. Alexander Löser, Paul Grundmann, Alexei Figueroa
...
`main.py` provides the code used to run the chats with the different LLMs.
...
```bash
python src/main.py --model mistralai/Mistral-7B-Instruct-v0.3 --companies Airbus --ratios current_ratio cash_ratio
```
...
├── results
    ├── csv-files                                   -- preparing datasets
    ├── figs                                        -- tuning functions
    └── model_outputs                               -- run experiments
└── src
